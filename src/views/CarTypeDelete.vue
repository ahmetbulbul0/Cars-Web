...
export default {
    ...
    methods: {
        ...
        deleteItem() {
            axios.delete(`http://127.0.0.1:8000/api/car-type/${this.item.id}`)
                .then(() => {
                    store.commit("setCarTypeDeletedMessage", this.item.name);
                    this.$router.push({ name: 'CarTypes' });
                })
                .catch(error => {
            ...
            <div class="w-full flex justify-center mt-6" v-if="item != null">
                <div class="w-1/3 bg-gray-50 rounded-md p-4 flex flex-col gap-y-4">
                    ...
                    <div class="w-full flex justify-center items-center text-sm font-medium text-gray-600">
                        <span>If you confirm, "{{ item.name }}" car type will be deleted.</span>
                    </div>
